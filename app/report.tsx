  ...
  return (
    <View style={styles.sectionStack}>
      ...
      <View style={styles.noteCard}>
        ...
        <Text style={styles.noteText}>This report was generated by the embedded Rust engine. Inferred output remains reconstructed analysis and requires independent review.</Text>
      </View>
...
function Security({ report }: { report: AnalysisReport }) {
  ...
  if (!report.findings.length) {
    return <EmptySection icon="verified-user" title="No engine findings" body="The embedded engine did not emit a security finding for this bytecode and profile. This is not a security assurance." />;
  }
  return (
    <View style={styles.sectionStack}>
      {report.findings.map((finding) => {
        const severity = finding.severity.toUpperCase();
        const color = severity === "CRITICAL" ? "#F35D5D" : severity === "HIGH" ? "#F98254" : severity === "MEDIUM" ? "#F4B942" : "#2DD4E9";
        return (
...
function Functions({ report }: { report: AnalysisReport }) {
  if (!report.functions.length) {
    return <EmptySection icon="functions" title="No selectors recovered" body="The embedded engine did not recover a dispatcher selector for this bytecode and profile." />;
  }
  return (
    <View style={styles.sectionStack}>
...
function Source({ report }: { report: AnalysisReport }) {
  return (
    <View style={styles.sectionStack}>
      <View style={styles.codeHeader}><Text style={styles.codeHeaderText}>PSEUDO-SOLIDITY</Text><Text style={styles.codeHeaderText}>EMBEDDED ENGINE OUTPUT</Text></View>
      ...
      <Text style={styles.sourceNote}>Recovered source is generated by the embedded engine’s bounded reconstruction pipeline. It is not verified source code or a proof of safety.</Text>
    </View>
  );
}
    ...
    <ScreenContainer edges={["top", "bottom", "left", "right"]} containerClassName="bg-background" className="px-5">
      ...
      <View style={[styles.riskCard, { backgroundColor: risk.surface, borderColor: `${risk.color}55` }]}>
        <View>
          ...
          <Text style={styles.riskDescription}>{report.findings.length ? `${report.findings.length} engine finding${report.findings.length === 1 ? "" : "s"} need attention` : "No engine findings in this analysis mode"}</Text>
        </View>
