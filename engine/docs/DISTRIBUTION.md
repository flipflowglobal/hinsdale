...
The Rust engine emits `hinsdale.report/v2`. Consumers must check `schema_version`, `metadata.analysis_profile`, and `capabilities.limitation` before relying on an analysis. The default Fast profile is a triage mode. Precise and Research increase bounded reconstruction effort but do not prove correctness, source equivalence, or the absence of security issues.
