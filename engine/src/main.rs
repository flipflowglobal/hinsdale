...
║  --tier <name>   fast | precise | research (default: fast)    ║
║  --schema        print the report schema version               ║
╚═══════════════════════════════════════════════════════════════╝"
    );
}
...
fn main() {
    ...
    let schema_only = args.iter().any(|a| a == "--schema");
    let quality_tier = args
        .windows(2)
    ...
    if schema_only {
        println!("{}", hinsdale::analysis::JSON_SCHEMA_VERSION);
        return;
    }
    ...
    let bytecode: Vec<u8> = {
        if let Some(pos) = args.iter().position(|a| a == "--file") {
            ...
            let hex_arg = args
                ...
                .find(|(index, value)| {
                    !value.starts_with('-')
                        && args.get(index - 1).map(|previous| previous.as_str()) != Some("--tier")
                })
                .map(|(_, value)| value);
    ...
    // ── Run analysis ───────────────────────────────────────────────────────
    let report = hinsdale::analyze_with_options(
        &bytecode,
        hinsdale::analysis::AnalysisOptions::for_tier(quality_tier),
    );
    ...
    if summary_only {
        println!(
            "HINSDALE [{}] │ {} bytes │ {} instrs │ {} blocks │ {} resolved jumps │ {} unresolved │ {} fns │ {} findings │ risk={}  ({:.1}ms)",
            report.metadata.analysis_profile,
            ...
            report.cfg_summary.unresolved_jump_count,
    ...
    println!("  Analysis tier   : {}", report.metadata.analysis_profile);
    println!(
        "  Bytecode size   : {} bytes ({} hex chars)",
        report.metadata.bytecode_len,
        report.metadata.bytecode_len * 2
    );
    println!(
        "  Runtime code    : {}",
        if report.metadata.is_runtime {
            "YES"
        } else {
            "NO (may be creation bytecode)"
        }
    );
    ...
    println!("  Limitation      : {}", report.capabilities.limitation);
    println!();
...
fn print_disassembly(report: &hinsdale::HinsdaleReport) {
    ...
    for ins in &report.disassembly.instructions {
        ...
        println!(
            "  {marker} 0x{:04x}  {:02x}  {:<14}{}{}",
            ins.offset, ins.opcode, ins.mnemonic, imm, jump_ann
        );
    }
    println!();
}
