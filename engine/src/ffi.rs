...
fn fill_summary(report: &HinsdaleReport, out: &mut HinsSummary) {
    let mut hint = [0u8; 64];
    if let Some(ref h) = report.metadata.solc_version_hint {
        copy_str(&mut hint, h);
    }
...
pub extern "C" fn hins_summary(ctx: *const HinsdaleCtx, out: *mut HinsSummary) -> c_int {
    let report = match unsafe { ctx_ref(ctx) } {
        Some(r) => r,
        None => return -1,
    };
    if out.is_null() {
...
pub extern "C" fn hins_instr_count(ctx: *const HinsdaleCtx) -> u32 {
    unsafe { ctx_ref(ctx) }
        .map(|r| r.disassembly.instruction_count as u32)
        .unwrap_or(0)
}
...
pub extern "C" fn hins_instr_at(ctx: *const HinsdaleCtx, idx: u32, out: *mut HinsInstr) -> c_int {
    ...
    let ins = match report.disassembly.instructions.get(idx as usize) {
        Some(i) => i,
        None => return -1,
    };
    if out.is_null() {
...
#[no_mangle]
pub extern "C" fn hins_instr_bulk(
...
pub extern "C" fn hins_batch_analyze(
    ...
    if slices.is_null() || out_ctxs.is_null() || count == 0 {
        return 0;
    }
    ...
    let results: Vec<usize> = inputs
        ...
        .map(|s| {
            ...
            let report = analyze(slice);
            Box::into_raw(Box::new(HinsdaleCtx(report))) as usize
        })
        .collect();
    ...
    for (i, ctx_ptr) in results.iter().enumerate() {
        unsafe {
            *out_ctxs.add(i) = *ctx_ptr as *mut HinsdaleCtx;
        }
    }
...
pub extern "C" fn hins_stream_disasm(
    ...
    for chunk_slice in instrs.chunks(chunk) {
        ...
        let cont = unsafe { callback(buf.as_ptr(), buf.len() as u32, user_data) };
...
pub extern "C" fn hins_fn_count(ctx: *const HinsdaleCtx) -> u32 {
    unsafe { ctx_ref(ctx) }
        .map(|r| r.signatures.functions.len() as u32)
        .unwrap_or(0)
}
...
pub extern "C" fn hins_fn_at(ctx: *const HinsdaleCtx, idx: u32, out: *mut HinsFnSig) -> c_int {
    ...
    let sig = match report.signatures.functions.get(idx as usize) {
        Some(s) => s,
        None => return -1,
    };
    if out.is_null() {
    ...
    unsafe {
        ...
        if let Some(ref n) = sig.known_name {
            copy_str(&mut o.known_name, n);
        } else {
            o.known_name[0] = 0;
        }
...
pub extern "C" fn hins_finding_count(ctx: *const HinsdaleCtx) -> u32 {
    unsafe { ctx_ref(ctx) }
        .map(|r| r.security.findings.len() as u32)
        .unwrap_or(0)
}

#[no_mangle]
pub extern "C" fn hins_finding_at(
    ctx: *const HinsdaleCtx,
    idx: u32,
    out: *mut HinsFinding,
) -> c_int {
    ...
    let finding = match report.security.findings.get(idx as usize) {
        Some(f) => f,
        None => return -1,
    };
    if out.is_null() {
...
#[no_mangle]
pub extern "C" fn hins_sizeof_instr() -> u32 {
    std::mem::size_of::<HinsInstr>() as u32
}

#[no_mangle]
pub extern "C" fn hins_sizeof_summary() -> u32 {
    std::mem::size_of::<HinsSummary>() as u32
}
