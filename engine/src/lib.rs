...
pub struct CfgSummary {
    ...
    pub resolved_jump_count: usize,
    pub unresolved_jump_count: usize,
}
...
pub fn analyze(bytecode: &[u8]) -> HinsdaleReport {
    analyze_with_options(bytecode, AnalysisOptions::default())
}

/// Run the full Hinsdale analysis pipeline using a declared execution profile.
pub fn analyze_with_options(bytecode: &[u8], options: AnalysisOptions) -> HinsdaleReport {
    let t0 = Instant::now();
    ...
    let cfg_summary = CfgSummary {
        ...
        resolved_jump_count: cfg.resolved_jump_count(),
        unresolved_jump_count: cfg.control.unresolved_jumps,
    };
    ...
    let private_functions = if options.quality_tier == analysis::QualityTier::Fast {
        vec![]
    } else {
        private_functions::recover_private_functions(&cfg, &signatures)
    };

    // 5. Decompile
    let decompiled = decompiler::decompile(
        &disassembly,
        &cfg,
        &signatures,
        &private_functions,
    ...
    let is_runtime =
        bytecode.len() >= 3 && bytecode[0] == 0x60 && bytecode[1] == 0x80 && bytecode[2] == 0x60;

    let is_erc20_like = signatures.functions.iter().any(|f| {
        f.known_name
            .as_deref()
            .map(|n| n.contains("transfer") || n.contains("balanceOf") || n.contains("approve"))
            .unwrap_or(false)
    });
    ...
    let is_erc721_like = signatures.functions.iter().any(|f| {
        f.known_name
            .as_deref()
            .map(|n| n.contains("tokenURI") || n.contains("ownerOf") || n.contains("safeTransfer"))
            .unwrap_or(false)
    });
