...
use crate::types::{CalldataParam, EvmType, TypeCtx};
...
impl Val {
    ...
    pub fn fold(&self) -> Val {
        match self {
            ...
            Val::Shr { shift, value } => match (shift.fold(), value.fold()) {
                ...
                (Val::Const(96), fv) => {
                    Val::And(Box::new(fv), Box::new(Val::Unknown("address_mask".into())))
                } // >> 96 = extract address
                (fs, fv) => Val::Shr {
                    shift: Box::new(fs),
                    value: Box::new(fv),
    ...
    pub fn render(&self) -> String {
        match self {
            ...
            Val::CalldataLoad { offset } => match offset.as_ref() {
            ...
            Val::Sar { shift, value } => {
                format!("(int256({}) >> {})", value.render(), shift.render())
            }
            ...
            Val::Keccak256 { offset, length } => match (offset.as_ref(), length.as_ref()) {
                (Val::Const(0x00), Val::Const(64)) => "keccak256(abi.encode(_key, _slot))".into(),
                (o, l) => format!(
                    "keccak256(memory[{}..{}+{}])",
                    o.render(),
                    o.render(),
            ...
            Val::CallSuccess { gas, to, value } => {
                ...
                format!(
                    "{}{{gas: {}{}}}.call(abi.encodeWithSelector(...))",
                    to.render(),
                    gas.render(),
                    value_part
                )
            }
            Val::StaticCallSuccess { gas, to } => format!(
                "{}.staticcall(...)  /* gas: {} */",
                to.render(),
                gas.render()
            ),
    ...
    pub fn ascii_string(&self) -> Option<String> {
        let Val::Const256(bytes) = self else {
            return None;
        };
        let printable: Vec<u8> = bytes.iter().copied().filter(|byte| *byte != 0).collect();
        if printable.len() < 3
            || !printable
                .iter()
...
impl SymStack {
    ...
    /// Merge predecessor stacks. Divergent values remain visible as phi-style values.
    pub fn merge(&self, incoming: &SymStack) -> SymStack {
        let max_depth = self.items.len().max(incoming.items.len());
        let mut items = Vec::with_capacity(max_depth);
        for index in 0..max_depth {
            match (self.items.get(index), incoming.items.get(index)) {
                (Some(left), Some(right)) if left == right => items.push(left.clone()),
                (Some(left), Some(right)) => {
                    items.push(Val::Phi(vec![left.clone(), right.clone()]))
                }
                (Some(left), None) => items.push(Val::Phi(vec![
                    left.clone(),
                    Val::Unknown("missing_predecessor".into()),
                ])),
                (None, Some(right)) => items.push(Val::Phi(vec![
                    Val::Unknown("missing_predecessor".into()),
                    right.clone(),
                ])),
                (None, None) => {}
...
impl<'a> SymExec<'a> {
    pub fn new(disasm: &'a Disassembly, cfg: &'a CFG) -> Self {
        Self::with_visit_limit(disasm, cfg, 1)
    }

    pub fn with_visit_limit(
        disasm: &'a Disassembly,
        cfg: &'a CFG,
        max_block_visits: usize,
    ) -> Self {
        Self {
            ...
            max_block_visits: max_block_visits.max(1),
    ...
    pub fn exec_block(&mut self, block_id: usize, mut stack: SymStack) -> BlockResult {
        ...
        for &idx in &block.instructions {
            ...
            match op {
                ...
                0x60..=0x7f => match ins.imm_u256 {
                    Some(v) => stack.push(Val::Const(v)),
                    None => {
                        let bytes =
                            hex::decode(ins.imm.as_deref().unwrap_or("")).unwrap_or_default();
                ...
                0x01 => {
                    let a = stack.pop();
                    let b = stack.pop();
                    stack.push(Val::Add(Box::new(a), Box::new(b)));
                }
                0x02 => {
                    let a = stack.pop();
                    let b = stack.pop();
                    stack.push(Val::Mul(Box::new(a), Box::new(b)));
                }
                0x03 => {
                    let a = stack.pop();
                    let b = stack.pop();
                    stack.push(Val::Sub(Box::new(a), Box::new(b)));
                }
                0x04 => {
                    let a = stack.pop();
                    let b = stack.pop();
                    stack.push(Val::Div(Box::new(a), Box::new(b)));
                }
                0x05 => {
                    let a = stack.pop();
                    let b = stack.pop();
                ...
                0x06 => {
                    let a = stack.pop();
                    let b = stack.pop();
                    stack.push(Val::Mod(Box::new(a), Box::new(b)));
                }
                0x08 => {
                    let a = stack.pop();
                    let b = stack.pop();
                    let n = stack.pop();
                    stack.push(Val::AddMod(Box::new(a), Box::new(b), Box::new(n)));
                }
                0x0a => {
                    let a = stack.pop();
                    let b = stack.pop();
                    stack.push(Val::Exp(Box::new(a), Box::new(b)));
                }
                0x0b => {
                    let _b = stack.pop();
                    let x = stack.pop();
                ...
                0x16 => {
                    let a = stack.pop();
                    let b = stack.pop();
                    stack.push(Val::And(Box::new(a), Box::new(b)));
                }
                0x17 => {
                    let a = stack.pop();
                    let b = stack.pop();
                    stack.push(Val::Or(Box::new(a), Box::new(b)));
                }
                0x18 => {
                    let a = stack.pop();
                    let b = stack.pop();
                    stack.push(Val::Xor(Box::new(a), Box::new(b)));
                }
                0x19 => {
                    let a = stack.pop();
                    stack.push(Val::Not(Box::new(a)));
                }
                0x1a => {
                    let i = stack.pop();
                    let v = stack.pop();
                ...
                0x10 => {
                    let a = stack.pop();
                    let b = stack.pop();
                    stack.push(Val::Lt(Box::new(a), Box::new(b)));
                }
                0x11 => {
                    let a = stack.pop();
                    let b = stack.pop();
                    stack.push(Val::Gt(Box::new(a), Box::new(b)));
                }
                0x12 => {
                    let a = stack.pop();
                    let b = stack.pop();
                    stack.push(Val::Slt(Box::new(a), Box::new(b)));
                }
                0x13 => {
                    let a = stack.pop();
                    let b = stack.pop();
                    stack.push(Val::Sgt(Box::new(a), Box::new(b)));
                }
                0x14 => {
                    let a = stack.pop();
                    let b = stack.pop();
                    stack.push(Val::Eq(Box::new(a), Box::new(b)));
                }
                0x15 => {
                    let a = stack.pop();
                    stack.push(Val::IsZero(Box::new(a)));
                }
                ...
                0x31 => {
                    let _a = stack.pop();
                    stack.push(Val::Unknown("addr.balance".into()));
                }
                ...
                0x35 => {
                    ...
                    if let Some(off) = offset.as_const() {
                        let param_idx = if off == 0 {
                            0
                        } else {
                            ((off.saturating_sub(4)) / 32) as usize
                        };
                        if off >= 4 || off == 0 {
                ...
                0x3b => {
                    let _a = stack.pop();
                    stack.push(Val::Unknown("extcodesize".into()));
                }
                0x3c => {
                    stack.pop();
                    stack.pop();
                ...
                0x3f => {
                    let _a = stack.pop();
                    stack.push(Val::Unknown("extcodehash".into()));
                }

                // ── BLOCK ────────────────────────────────────────────────
                0x40 => {
                    let _b = stack.pop();
                    stack.push(Val::Unknown("blockhash".into()));
                }
                ...
                0x49 => {
                    let _i = stack.pop();
                    stack.push(Val::Unknown("blobhash".into()));
                }
                ...
                0x51 => {
                    ...
                    let val = if let Some(off) = o.as_const() {
                        mem.get(&off).cloned().unwrap_or(Val::MLoad {
                            offset: Box::new(o.clone()),
                        })
                    } else {
                        Val::MLoad {
                            offset: Box::new(o),
                ...
                0x54 => {
                    ...
                    if matches!(&slot, Val::Keccak256 { .. }) {
                        self.type_ctx.record_mapping(slot_str.clone(), false);
                    }
                    stmts.push(Stmt::Comment(format!("SLOAD {slot_str}")));
                    stack.push(Val::SLoad {
                        slot: Box::new(slot),
                    });
                }
                0x55 => {
                    ...
                    if matches!(&slot, Val::Keccak256 { .. }) {
                        self.type_ctx.record_mapping(slot.render(), true);
                    }
                ...
                0x5c => {
                    let _s = stack.pop();
                    stack.push(Val::Unknown("tload".into()));
                }
                0x5d => {
                    stack.pop();
                    stack.pop();
                    ...
                    if is_negated {
                        ...
                        if let Val::IsZero(inner) = &cond {
                            ...
                            if jumps_to_revert {
                                ...
                                let reason = mem.get(&4).map(|value| {
                                    value.ascii_string().unwrap_or_else(|| value.render())
                                });
                ...
                0xf3 => {
                    ...
                    let value = match (ofs.as_const(), len.as_const()) {
                        (Some(0), Some(0)) => "()".into(),
                        (Some(o), _) => mem.get(&o).map(|v| v.render()).unwrap_or_else(|| {
                            format!(
                                "memory[{}..{}+{}]",
                                ofs.render(),
                                ofs.render(),
                ...
                _ => {
                    ...
                    // Consume inputs
                    for _ in 0..(def.stack_in.max(0) as usize) {
                        stack.pop();
                    }
                    // Produce outputs
                    for j in 0..(def.stack_out.max(0) as usize) {
                        stack.push(Val::Unknown(format!(
                            "{}_{}",
                            def.mnemonic.to_lowercase(),
                            j
                        )));
                    }
                }
        ...
        let stmts = stmts
            .into_iter()
            .filter(|s| !matches!(s, Stmt::MStore { .. }))
            .collect();
    ...
    /// Execute the CFG with bounded revisits. Divergent predecessor stacks become phi values
    /// instead of being silently discarded by traversal order.
    pub fn run(&mut self) -> FxHashMap<usize, BlockResult> {
        ...
        while let Some((block_id, stack)) = queue.pop_front() {
            let visit_count = self.visit_counts.entry(block_id).or_insert(0);
            if *visit_count >= self.max_block_visits {
                self.stats.visit_limit_hits += 1;
                continue;
            }
            *visit_count += 1;

            let merged_stack = match self.incoming_stacks.get(&block_id) {
                Some(previous) if previous != &stack => {
                    self.stats.merged_inputs += 1;
                    previous.merge(&stack)
                }
                Some(previous) => previous.clone(),
                None => stack,
            };
            self.incoming_stacks.insert(block_id, merged_stack.clone());

            let result = self.exec_block(block_id, merged_stack);
            self.stats.executed_blocks += 1;
            let successors = self.cfg.blocks[block_id].successors.clone();

            for &succ in &successors {
                self.stats.explored_paths += 1;
                let next_stack = match self.incoming_stacks.get(&succ) {
                    Some(previous) => previous.merge(&result.stack_out),
                    None => result.stack_out.clone(),
                };
                let changed = self
                    .incoming_stacks
                    .get(&succ)
                    .map(|previous| previous != &next_stack)
                    .unwrap_or(true);
                if changed {
                    queue.push_back((succ, next_stack));
                }
            }
