...
fn builtin_event(topic: &str) -> Option<&'static str> {
    match topic {
        "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef" => {
            Some("Transfer(address,address,uint256)")
        }
        "0x8be0079c531659141344cd1fd0a4f28419497f9723d4d2c2dcf6a93cbb4f0c0c" => {
            Some("OwnershipTransferred(address,address)")
        }
        _ => None,
    }
}
...
pub fn recover_signatures(disasm: &Disassembly) -> SignatureReport {
    ...
    let mut events: Vec<EventTopic> = Vec::new();
                        ...
                        window
                            ...
                            .and_then(|_| {
                                ...
                                window
                                    .windows(2)
                                    .find(|w| {
                                        w[1].opcode == 0x57 && (0x60..=0x7f).contains(&w[0].opcode)
                                    })
            ...
            if nearby_log {
                if let Some(ref hex) = ins.imm {
                    if !event_topics.contains(hex) {
                        let topic = format!("0x{}", hex);
                        let known_name = builtin_event(&topic).map(str::to_string);
                        events.push(EventTopic {
                            topic: topic.clone(),
                            confidence: if known_name.is_some() { 0.98 } else { 0.45 },
                            known_name,
                        });
