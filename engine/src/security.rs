    ...
    for (i, ins) in instrs.iter().enumerate() {
        match ins.opcode {
            ...
            0xf4 => {
                ...
                let addr_from_calldata = instrs[..i]
                    .iter()
                    .rev()
                    .take(10)
                    .any(|x| x.opcode == 0x35 || x.opcode == 0x36);
                let sev = if addr_from_calldata {
                    Severity::Critical
                } else {
                    Severity::High
                };
            ...
            0x55 => {
                ...
                let call_before_sstore = instrs[..i]
                    .iter()
                    .rev()
                    .take(50)
                    .any(|x| x.opcode == 0xf1 || x.opcode == 0xf4);
                let sstore_before_call =
                    instrs[..i].iter().rev().take(50).any(|x| x.opcode == 0x55);
