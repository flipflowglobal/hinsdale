...
impl EvmType {
    pub fn solidity_name(&self) -> String {
        match self {
            ...
            Self::Mapping(k, v) => {
                format!("mapping({} => {})", k.solidity_name(), v.solidity_name())
            }
...
/// A potential Solidity mapping access reconstructed from a keccak-based storage address.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub struct TypeCtx {
    ...
    /// keccak-derived storage expressions that may represent mappings
    pub mappings: FxHashMap<String, MappingCandidate>,
...
impl TypeCtx {
    ...
    pub fn record_mapping(&mut self, expression: String, is_write: bool) {
        let candidate = self
            .mappings
            .entry(expression.clone())
            .or_insert(MappingCandidate {
                expression,
                reads: 0,
                writes: 0,
                confidence: 0.58,
    ...
    pub fn to_storage_vars(&self) -> Vec<StorageVar> {
        ...
        let slot_names: FxHashMap<u64, (&str, EvmType)> = [
            ...
            (
                ...
                (
                    "allowances",
                    EvmType::Mapping(
                        Box::new(EvmType::Address),
                        Box::new(EvmType::Mapping(
                            Box::new(EvmType::Address),
                            Box::new(EvmType::Uint(256)),
                        )),
                    ),
                ),
            ),
        ...
        let mut vars: Vec<StorageVar> = self
            .storage
            .iter()
            .map(|(&slot, (ty, r, w))| {
                let (name, inferred_ty) = slot_names
                    .get(&slot)
    ...
    pub fn mapping_candidates(&self) -> Vec<MappingCandidate> {
        let mut candidates: Vec<_> = self.mappings.values().cloned().collect();
        candidates.sort_by(|left, right| left.expression.cmp(&right.expression));
        candidates
    }
}
