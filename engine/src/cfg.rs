...
pub struct ControlFlowSummary {
    ...
    pub join_blocks: Vec<usize>,
    pub switch_like_blocks: Vec<usize>,
...
pub struct CFG {
    ...
    pub control: ControlFlowSummary,
}
...
impl CFG {
    ...
    pub fn resolved_jump_count(&self) -> usize {
        self.control
            .edges
            .iter()
            .filter(|edge| matches!(edge.kind, CfgEdgeKind::StaticJump))
            .count()
    }
}
...
fn resolve_stack_local_target(
    last_idx: usize,
    instrs: &[Instruction],
    start_off_to_bid: &FxHashMap<usize, usize>,
) -> Option<(usize, f32, String)> {
    for distance in 1..=8 {
        let Some(index) = last_idx.checked_sub(distance) else {
            break;
        };
        let instruction = &instrs[index];
        let Some(target_offset) = instruction.imm_u256.map(|value| value as usize) else {
            continue;
        };
        let Some(&target_bid) = start_off_to_bid.get(&target_offset) else {
            continue;
        };
        let confidence = if distance == 1 {
            0.98
        } else {
            (0.76 - (distance as f32 * 0.04)).max(0.5)
        };
        let evidence = if distance == 1 {
            "direct PUSH before JUMP"
        } else {
            "stack-local PUSH candidate before JUMP"
        };
        return Some((target_bid, confidence, evidence.into()));
    }
    None
}
...
pub fn build_cfg(disasm: &Disassembly) -> CFG {
    ...
    for (bid, block) in blocks.iter().enumerate() {
        ...
        if has_fallthrough && bid + 1 < block_count {
            ...
            edges.push(CfgEdge {
                ...
                confidence: 1.0,
                evidence: "sequential instruction flow".into(),
            });
        }

        // JUMP / JUMPI: resolve a direct or stack-local constant target and preserve unresolved evidence.
        if is_jump(last_op) {
            if let Some((target_bid, confidence, evidence)) =
                resolve_stack_local_target(last_idx, instrs, &start_off_to_bid)
            {
                ...
                edges.push(CfgEdge {
                    from: bid,
                    to: None,
                    kind: CfgEdgeKind::DynamicUnresolved,
                    confidence: 0.0,
                    evidence: "jump operand is not statically propagated".into(),
                });
            }
        }
    }
    ...
    let loops = edges
        ...
        .filter_map(|edge| {
            let target = edge.to?;
            if matches!(edge.kind, CfgEdgeKind::StaticJump) && target <= edge.from {
                Some(LoopSummary {
                    header_block: target,
                    back_edge_from: edge.from,
    ...
    let join_blocks = blocks
        ...
        .map(|block| block.id)
        .collect();
    let switch_like_blocks = blocks
        .iter()
        .filter(|block| block.successors.len() > 1 && block.predecessors.len() <= 1)
        .map(|block| block.id)
        .collect();
    let unresolved_jumps = edges
        .iter()
        .filter(|edge| matches!(edge.kind, CfgEdgeKind::DynamicUnresolved))
        .count();
